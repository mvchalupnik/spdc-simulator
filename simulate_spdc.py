...
def monte_carlo_integration_momentum(f, dqsx, dqsy, dqix, dqiy, num_samples):
    ...
    for j in range(num_samples // batch_size):
        ...
        average = (
            np.mean(func_values) if j == 0 else np.mean([average, np.mean(func_values)])
        )  # Make this into a list for parallelization
...
def adaptive_integration_momentum(
    f, dqsx, dqsy, dqix, dqiy, num_samples_coarse, num_samples_fine
):
    """
    ...
    dqix_grid, dqiy_grid, dqsx_grid, dqsy_grid = np.meshgrid(
        dqix_samples, dqiy_samples, dqsx_samples, dqsy_samples, indexing="ij"
    )
    ...
    coarse_coords_indices = np.random.choice(
        range(len(thresholded_coord_pairs)), size=num_samples_fine, replace=True
    )
    coarse_coords = thresholded_coord_pairs[
        coarse_coords_indices
    ]  # TODO they aren't pairs, rename
    grid_width_fraction = 0.1
    dqsx_samples_fine = (
        np.random.uniform(
            -grid_width_fraction * dqsx_grid_width,
            grid_width_fraction * dqsx_grid_width,
            num_samples_fine,
        )
        + coarse_coords[:, 0]
    ...
    dqsy_samples_fine = (
        np.random.uniform(
            -grid_width_fraction * dqsy_grid_width,
            grid_width_fraction * dqsy_grid_width,
            num_samples_fine,
        )
        + coarse_coords[:, 1]
    ...
    dqix_samples_fine = (
        np.random.uniform(
            -grid_width_fraction * dqix_grid_width,
            grid_width_fraction * dqix_grid_width,
            num_samples_fine,
        )
        + coarse_coords[:, 2]
    ...
    dqiy_samples_fine = (
        np.random.uniform(
            -grid_width_fraction * dqiy_grid_width,
            grid_width_fraction * dqiy_grid_width,
            num_samples_fine,
        )
        + coarse_coords[:, 3]
    ...
    thresholded_func_values = f(
        dqix_samples_fine, dqiy_samples_fine, dqsx_samples_fine, dqsy_samples_fine
    )
...
def grid_integration_position(
    f, dqix, dqiy, dqsx, dqsy, dx, dy, num_samples_position, num_samples_momentum
):
    """
    ...
    # Evaluate the function at each sample point
    func_values = np.zeros(
        len(coord_pairs), dtype="complex128"
    )  # Technically won't be complex here, also TODO rewrite faster way with ravel
    for n in range(len(coord_pairs)):
        ...
        func_values[n] = monte_carlo_integration_momentum(
            g, dqsx, dqsy, dqix, dqiy, num_samples_momentum
        )
...
def alpha(thetap, lambdap):
    ...
    """
    alpha = (
        (n_o(lambdap) ** 2 - n_e(lambdap) ** 2) * np.sin(thetap) * np.cos(thetap)
    ) / (
        n_o(lambdap) ** 2 * np.sin(thetap) ** 2
        + n_e(lambdap) ** 2 * np.cos(thetap) ** 2
    )
    return alpha
...
def eta(thetap, lambdap):
    ...
    :param lambdap: Pump wavelength, in meters
    """
    eta = (n_o(lambdap) * n_e(lambdap)) / np.sqrt(
        (
            n_o(lambdap) ** 2 * np.sin(thetap) ** 2
            + n_e(lambdap) ** 2 * np.cos(thetap) ** 2
        )
...
def get_rate_integrand(x_pos, y_pos, thetap, omegai, omegas, simulation_parameters):
    ...
    omegap = (
        omegas + omegai
    )  # This is on page 8 in the bottom paragraph on the left column
    ...
    def rate_integrand(
        qix, qiy, qsx, qsy, x_pos_integrate, y_pos_integrate, integrate_over
    ):
        if integrate_over == "signal":  # TODO fix to be less confusing
            ...
            raise ValueError(
                f"Expecting to integrate over signal or idler and received {integrate_over}."
            )
        ...
        integrand = (
            ...
            * phase_matching(
                delta_k_type_1(qsx, qix, qsy, qiy, thetap, omegap, omegai, omegas),
                crystal_length,
            )
            * np.exp(
                1j
...
def calculate_conditional_probability(
    ...
    rate_integrand = get_rate_integrand(
        xs_pos, ys_pos, thetap, omegai, omegas, simulation_parameters
    )
    rate_integrand_signal = functools.partial(
        rate_integrand,
        integrate_over="idler",
        x_pos_integrate=xi_pos,
        y_pos_integrate=yi_pos,
    )
    ...
    result_signal = adaptive_integration_momentum(
        f=rate_integrand_signal,
        dqsx=dqsx,
        dqsy=dqsy,
        dqix=dqix,
        dqiy=dqiy,
        num_samples_coarse=25,
        num_samples_fine=40000,
    )
...
def calculate_pair_generation_rate(
    x_pos, y_pos, thetap, omegai, omegas, dx, dy, integrate_over, simulation_parameters
):
    """
    ...
    rate_integrand = get_rate_integrand(
        x_pos, y_pos, thetap, omegai, omegas, simulation_parameters
    )
    rate_integrand_wrt = functools.partial(
        rate_integrand, integrate_over=integrate_over
    )
    result = grid_integration_position(
        f=rate_integrand_wrt,
        dqix=dqix,
        dqiy=dqiy,
        dqsx=dqsx,
        dqsy=dqsy,
        ...
        num_samples_momentum=num_samples_monte_carlo,
    )
...
def simulate_ring_momentum(simulation_parameters):
    ...
    rate_integrand = get_rate_integrand(
        signal_x_pos, signal_y_pos, thetap, omegai, omegas, simulation_parameters
    )
...
def simulate_ring_slice(simulation_parameters):
    ...
    x_signal_span = simulation_parameters[
        "signal_x_span"
    ]  # Span in the x-direction to plot conditional probability of signal over, in meters
    signal_y_pos = simulation_parameters["signal_y_pos"]
    idler_x_span = simulation_parameters[
        "idler_x_span"
    ]  # Span in the x-direction to fix idler at
    idler_x_increment = simulation_parameters[
        "idler_x_increment"
    ]  # Increment size to change idler by in the x-direction
    idler_y_pos = simulation_parameters["idler_y_pos"]
    ...
    x_signal = np.linspace(
        -x_signal_span, x_signal_span, num_plot_x_points
    )  # TODO standardize x_signal or signal_x
    sweep_points = np.arange(-idler_x_span, idler_x_span, idler_x_increment)

    calculate_conditional_probability_vec = np.vectorize(
        calculate_conditional_probability
    )
    parallel_calc_conditional_prob = functools.partial(
        calculate_conditional_probability_vec,
        yi_pos=idler_y_pos,
        thetap=thetap,
        omegai=omegai,
        omegas=omegas,
    ...
    z1 = Parallel(n_jobs=num_cores)(
        delayed(parallel_calc_conditional_prob)(x_signal, signal_y_pos, idler_x_pos)
        for idler_x_pos in sweep_points
    )
    ...
    plt.title(
        "Conditional probability of signal given idler at different locations on x-axis"
    )
...
def simulate_rings(simulation_parameters):
    ...
    x_span = simulation_parameters[
        "x_span"
    ]  # Span in the x-direction to plot over, in meters
    y_span = simulation_parameters[
        "y_span"
    ]  # Span in the y-direction to plot over, in meters
    thetap = simulation_parameters["thetap"]  # Incident pump angle, in Radians
    omegap = simulation_parameters["omegap"]  # Pump frequency (Radians / sec)
    omegai = simulation_parameters["omegai"]  # Idler frequency (Radians / sec)
    omegas = simulation_parameters["omegas"]  # Signal frequency (Radians / sec)
    momentum_span = simulation_parameters[
        "momentum_span"
    ]  # Extent of span to integrate in momentum space over across x axis and in y axis for both signal and idler (fraction of omega / C)
    num_momentum_integration_points = simulation_parameters[
        "num_momentum_integration_points"
    ]  # Number of points to integrate over in momentum space
    grid_integration_size = simulation_parameters[
        "grid_integration_size"
    ]  # Size of square root of grid for integration in real space (todo improve name)
    ...
    num_cores = simulation_parameters[
        "simulation_cores"
    ...
    parallel_calculate_pair_generation_rate = functools.partial(
        calculate_pair_generation_rate_vec,
        thetap=thetap,
        omegai=omegai,
        omegas=omegas,
        dx=x_span,
    ...
    Z1 = Parallel(n_jobs=num_cores)(
        delayed(parallel_calculate_pair_generation_rate)(xs, ys) for xs in x for ys in y
    )
