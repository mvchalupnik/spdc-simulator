...
def get_rate_integrand_real(x_pos, y_pos, thetap, omegai, omegas, simulation_parameters):
    """
    ...
    def rate_integrand(q, x_pos_integrate, y_pos_integrate, integrate_over): # Move this, supply all params, combine with get_rate_integrand_imag TODO
        qix, qiy, qsx, qsy = q
...
def get_rate_integrand_imag(x_pos, y_pos, thetap, omegai, omegas, simulation_parameters): #TODO duplicate
    """
    Return the integrand used to calculate rates and conditional probabilities
    """
    # Also can multiply by detector efficiencies, and a constant dependent on epsilon_0 and chi_2
...
def calculate_conditional_probability(xs_pos, ys_pos, xi_pos, yi_pos, thetap, omegai, omegas, simulation_parameters):
    ...
    rate_integrand_real = get_rate_integrand_real(xs_pos, ys_pos, thetap, omegai, omegas, simulation_parameters)
    rate_integrand_imag = get_rate_integrand_imag(xs_pos, ys_pos, thetap, omegai, omegas, simulation_parameters)
...
def calculate_pair_generation_rate(x_pos, y_pos, thetap, omegai, omegas, dx, dy, integrate_over, simulation_parameters):
    ...
    rate_integrand_real_wrt = functools.partial(rate_integrand_real, integrate_over=integrate_over)
    rate_integrand_imag = get_rate_integrand_imag(x_pos, y_pos, thetap, omegai, omegas, simulation_parameters)
...
def simulate_ring_momentum(simulation_parameters):
    ...
    num_plot_qy_points = simulation_parameters["num_plot_qy_points"] #TODO fix this
